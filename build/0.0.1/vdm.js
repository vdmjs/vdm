...
/***/ function(module, exports, __webpack_require__) {
	...
	var View = function (options) {
	  ...
	  if (("development") != 'production' && !this.$el) {
	    _.error('vdm need a root el and must be a element or virtual dom')
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	function _matchPatid(node,vdmId){
	  var nodeType = node.nodeType

	  if (!nodeType || !_.inArray([1,8],nodeType)) return false


	  if (nodeType === 8 && _.trim(node.data) === 'deleted-'+vdmId) {
	    return node
	  }

	  if (nodeType === 1 && node.getAttribute && parseInt(node.getAttribute(TAG_ID)) === vdmId) {
	    return node
	  }
	...
	/**
	 * 用来通过vdmId获取对应的节点
	 * @param  {[type]} vdmId [description]
	 * @return {[type]}       [description]
	 */
	exports.queryPatId = function(root,vdmId){
	  var node = null

	  exports.walk(root,function(child){
	    if (_matchPatid(child,vdmId)) {
	      node = child
	      return false
	    }
	  })
	...
	var _query = document.querySelector ? function(root,vdmId){

	  return root.querySelector('['+TAG_ID+'="' + vdmId + '"]')
	...
	exports.queryRealDom = function(virtualDom){
	  ...
	  if (!virtualDom.deleted) {
	    ...
	    if (!virtualDom.parentNode || virtualDom.parentNode.__ROOT__ || virtualDom.parentNode.nodeType === -1) {
	      ...
	      parent = _query(root,virtualDom.parentNode.vdmId)
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	if (true){
	  ...
	  exports.error = function(str,e) {
	    ...
	    str && console.error('[vdm-error]:' + str)

	    if (e && e instanceof Error) {
	      console.error('[vdm-error]:' + e.stack)
	    }
	  }
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Node = Class.extend(domProp,{
	  ...
	  mountView: function(view) {
	    ...
	    //所以这里可以直接认为有了vdmId后就已经生成了dom,可以拿到dom
	    self.mountId()
	  ...
	  getElement: function() {
	    ...
	    if (!self.vdmId || !self.view) return
	...
	var Collection = Element.extend({
	  ...
	  getElement: function() {
	    if (!this.vdmId || !this.view) return
	...
	var TextNode = Node.extend({
	  ...
	  mountHtml: function(view) {
	    ...
	    return '<span ' + TAG_ID + '="' + this.vdmId + '">' + this.data + '</span>'
	  }
	})
